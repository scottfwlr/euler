...
class Grid
  ...
  def greatest_adjacents(n, &f)
    # initialise candidates array
    # generate vertical, horizontal, diagonal adjacency subarrays
    # max_by(&f)

    # note: f must take an array and return something that can be sorted on
  end
...
# vertical test
(0..width).each { |x|
  (0..height-adjacents).each { |y|
    # candidate
    grid[x][y...y+adjacents].reduce(:*)
    }
  }
}
